...
void loadQuestionListWithTopicAndDifficulty(string FilePath,
                                            vector<Question> &QuestionList,
                                            string topic, char diff) {
  ifstream fin(FilePath);
  if (!fin.is_open())
    return;
  while (!fin.eof()) {
...
void editQuestionList(vector<Question> &QuestionList) {
  ...
  cin >> diff;
  loadQuestionListWithTopicAndDifficulty(QUESTION_PATH_DATA, QuestionList,
                                         topic, diff);
  for (int i = 0; i < QuestionList.size(); i++) {
    bool choice;
...
void createTopic() {
  ...
  cout << "How many question do you want to create in difficulty 1?(at least 5)"
       << endl;
  cin >> choice;
  if (choice < 5)
    choice = 5;
  ...
  system("cls");
  cout << "How many question do you want to create in difficulty 2?(at least 3)"
       << endl;
  cin >> choice;
  if (choice < 3)
    choice = 3;
  ...
  system("cls");
  cout << "How many question do you want to create in difficulty 3?(at least 2)"
       << endl;
  cin >> choice;
  if (choice < 2)
    choice = 2;
